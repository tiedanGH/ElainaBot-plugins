...
_CRITERIA = {
    ...
    'echo': ('用户输入回显 (代码行为)',
             ...
             '几十个字足够塞下违规内容。\n'
             '**不算违规**: 只发送代码自己生成的内容 (棋盘、点数、结算文本); 发送框架提供的'
             '玩家昵称与 @ (非玩家自填); 把输入**解析或校验成受限取值后**再输出该取值 '
             '(如 "1E5" 解析成坐标后播报「放置于 E5」、数字、枚举、白名单内的卡牌名) —— '
             ...
             '  - 校验失败时回**固定错误文案** (如 `reply() << "[错误] 位置无效"`), '
             '文案里不含输入内容;\n'
             '  - 拿输入做条件判断、查表、索引、算坐标, 但不发送它。\n'
             '**发送的是中间变量时, 必须追到赋值处**: 实参若不是玩家输入本身, 而是某个'
             '成员变量或缓存 (如 round_status_、last_msg_、战报字符串), 就要把**它的每一处'
             '赋值**都找出来。只有其中确有一处把玩家输入的字符串拼了进去, 才算 echo。'
             '若所有赋值都由棋局状态生成 (如 `x_ = board_.GetBoardHTML(camp)`, 而渲染函数'
             '只接受枚举/整数、棋盘结构里也没有字符串字段), 那它装不下玩家输入, **不是** echo。'
