...
DRAW_RULE = (
    ...
    '**要画的东西属于某个游戏时（角色、场景、道具、棋盘、卡面…），'
    '必须先 read_game_source / read_game_rule 读到该游戏里对它的真实描述，'
    '把读到的具体特征写进画面描述，再调用画图。**'
    '绝不允许只凭游戏名或印象去想象 —— 那样画出来的东西跟游戏毫无关系，'
    '而用户要的正是「这个游戏里的」。源码里确实没写外观时，'
    '就用规则里能确定的元素（棋盘形状、道具名称、阶段场景）作为依据，并在回答里说明。\n'
